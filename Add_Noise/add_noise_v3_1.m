function add_noise_v3_1(InpPath,NoiseInpPath,OutDir,SNR,INDICATOR)
...
switch lower(INDICATOR)
    case 'ts'
        [noise_data,nFs]=audioread(NoiseInpPath,'native');
        noise_data=double(noise_data);
...
noise_data_tmp=sqrt((speech_data'*speech_data)/(10^(SNR/10)))*noise_data(from_point:to_point)/sqrt(noise_data(from_point:to_point)'*noise_data(from_point:to_point));
out_speech_data=speech_data+noise_data_tmp;

audiowrite(OutDir,int16(round(out_speech_data)),Fs);
