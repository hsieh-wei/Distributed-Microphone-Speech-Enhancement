...
opt = Nadam(lr=0.00008, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004)
# n_classes = 2
feature_dim = 257
...
def ddae_generator_noVAD_1(Trpath_1, Trpath_2, Trpath_3, TrClPth , fs=16000, ws=3, Frame_size=512, Frame_shft=256):
    ...
    while True:
        ...
        Onydata_1=np.insert(Onydata_1, 1285, values=x, axis=1)
        Onydata_1=np.insert(Onydata_1, 1286, values=y, axis=1)
        Onydata_1=np.insert(Onydata_1, 1287, values=z, axis=1)
        ...
        Onydata_2=np.insert(Onydata_2, 1285, values=x, axis=1)
        Onydata_2=np.insert(Onydata_2, 1286, values=y, axis=1)
        Onydata_2=np.insert(Onydata_2, 1287, values=z, axis=1)
        ...
        Onydata_3=np.insert(Onydata_3, 1285, values=x, axis=1)
        Onydata_3=np.insert(Onydata_3, 1286, values=y, axis=1)
        Onydata_3=np.insert(Onydata_3, 1287, values=z, axis=1)
...
def ddae_generator_noVAD_2(Vapath_1, Vapath_2, Vapath_3, VaClPth ,  fs=16000, ws=512, Frame_size=512, Frame_shft=256):
    ...
    while True:
        ...
        z=axis_z_test[i]*np.ones((1,(np.size(Onydata_1,0))))
        Onydata_1=np.insert(Onydata_1, 1285, values=x, axis=1)
        Onydata_1=np.insert(Onydata_1, 1286, values=y, axis=1)
        Onydata_1=np.insert(Onydata_1, 1287, values=z, axis=1)
...
mat_data = scipy.io.loadmat('/workdir/107wei/matlab_simulation_data/source_position.mat')
source_position =mat_data['s']
    ...
    for j in range(3):
        ...
        for k in range(8):
            for l in range(300):
                num = str(l + 1) + '.wav'
                if j == 0:
                    path = '/workdir/107wei/matlab_simulation_data/train noise/'+db+'/'+ch+'/'+noise_list[k]+'/'+num
...
for i in range(64):               
    for j in range(300):
        num = str(j + 1) + '.wav'
        path = '/workdir/107wei/matlab_simulation_data/target/'+ num
        TrClPth.append(path)
    ...
    for j in range(3):
        ...
        for k in range(4):
            for l in range(20):
                num = str(l + 301) + '.wav'
                if j == 0:
                    path = '/workdir/107wei/matlab_simulation_data/test noise/'+db+'/'+ch+'/'+noise_list_2[k]+'/'+num
...
checkpointer = [
ModelCheckpoint(
filepath="./3dim_train_5percent.hdf5",
verbose=0,
save_best_only=True,
monitor="val_loss",
mode="min")]
...
matplotlib.pyplot.savefig('./3dim_train_5percent.hdf5' + '.png', dpi=150)

model_1.load_weights("./3dim_train_5percent.hdf5")
...
for i in range(len(VaClPth)):
    ...
    y_pr_wav = spec2wav(VaClPth[i], './output_3dim_train_5percent/'+str(count)+'/' + str((i+ 301)-((count-1)*20)) + '.wav', y_pr_1, hop_length=256)
    flag +=1